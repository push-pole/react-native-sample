...
import {setupPushPoleEventListeners, clearPushPoleEvents} from "../startup";
import packageJson from "pushpole-react-native/package.json";
...
export default () => {
    ...
    useEffect(() => {
        ...
        // we attach pushpole events here in this component
        // but it could be anywhere as long as it is top level and not changes a lot
        setupPushPoleEventListeners((label, value) => setState({label, value}));
  
        // clear pushpole event listeners on unmounting
        return clearPushPoleEvents;
    }, []);

    return (
        <View style={styles.container}>
        <Text style={styles.bigTitle}>PushPole SAMPLE</Text>
